...
namespace Graphics
{
	...
	std::unique_ptr<IDX12PSO> DX12PSOAbstractFactory::CreatePSO(DX12Device* pDX12Device, Shader::Shader* pShader)
	{
		...
		{
			if (!pDX12PSO->InitializeRootSignature(pDX12Device, pShader, graphicsPipelineStateDesc))
			{
				MessageBoxA(NULL, "ルートシグネチャの初期化に失敗しました。", "MessageBox", MB_OK);
				return nullptr;
			}
...

		{
			...
			if (!pDX12PSO->InitializePipeline(pDX12Device, graphicsPipelineStateDesc))
			{
				MessageBoxA(NULL, "グラフィックスパイプラインの初期化に失敗しました。", "MessageBox", MB_OK);
				return nullptr;
			}
		}

		return std::move(pDX12PSO);
	}
